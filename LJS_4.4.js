...
//Пример областью this стрел.функции в другой функции
{
    ...
    /*
         1. Создаем объект "obj", содержащий метод
            bar;
         2. Метод "bar" возвращает функцию 
         3. Функция возвращает свой "this"
         
         Возвращаемая функция создана как стрелочная
         функция. 

         "this" - посоянно замкнут на "this" метода

         Значение метода "bar" может быть установлено
         в вызове, который устанавливает значение 
         возвращаемой функции
    */

         let link_arrow = obj.bar();
         // Вызываем bar как метод объекта obj, 
         //       устанавливая его this на obj.
         // Присваеваем ссылку возвращаемой функции
         // переменной link_arrow

        //  console.log(link_arrow() === obj);// true
        // Вызываем link_arrow без установки this,
        //  что в обычных функциях указывало бы на 
        //  глобальный объект или undefined в строгом
        //  режиме.
        
            function usualFun () {
                return this;
            }
            // console.log(usualFun()); // window
            
            function strictFun() {
                'use strict';
                return this;
            }
            // console.log(strictFun()); // undefined

            // Если ссылаться на obj но не вызывать его
            let link_arrow2 = obj.bar;
            // console.log(link_arrow2());// ()=> this
            // console.log(link_arrow2()());// window
            // Вызов следует за "this" "link_arrow2"
}//--------------------------------------------------
...
//===================================================
/*
   Когда функция вызывается как метод объекта, исполь-
   зуемое в этой функции "this" принимает значение 
   объекта, по отношению которого вызван метод.
*/
// Пример
{
    let obj = {
    ...
    /*===============================================
        На поведение this в данном случае не влияет 
        то, как или где была определена функция.
    ================================================*/

// Пример с внешней функцией определённой как метод
{//--------------------------------------------------
        /*
       В предыдущем примере функция определена в obj
       во время определения объекта.
       Поведение this не изменится даже если сначало
       определить функцию, а затем закрепить её за
       свойством obj.method
    */
   let obj = {value: 37};
   ...
   obj.method = independet;

//    console.log(obj.method()); // 37
}//--------------------------------------------------

// Пример показывающий присваивание this на ближайший
// объект или св-во
{
    let obj = {value: 37}; 
    ...
    obj.close = {key: independet, value: 42};

    // console.log(obj.close.key());// 42
}//--------------------------------------------------
...
//===================================================
/*
   Когда функция используется как конструктор, this
   связано с создаваемым новым объектом.

   По умолчанию конструктор возвращает новый объект,
   на который ссылается this, но он может вернуть
   и другой объект.
*/
// Пример
{
    ...
    let exempCon = new Constructor1();

    // console.log(exempCon.value); // 37
    ...
    exempCon = new Constructor2();

    // console.log(exempCon.value); // 99

    /*
        В последнем примере Constructor2, из-за 
        того, что конструктор вернул объект, новый
        объект, к которому привязан this, был про-
        сто отброшен
     */
}
